...
class CodableFeedStoreTests: XCTestCase {
        ...
        sut.retrieve{ firstResult in
            sut.retrieve{ secondResult in
                ...
                case (.empty, .empty):
                    break
                default:
                    XCTFail("Expected retrieving twice to deliver empty results, got \(firstResult) and \(secondResult) instead")
                }
                exp.fulfill()
            }
        }
        ...
        sut.insert(feed, timestamp: timestamp){ insertionError in
            ...
            sut.retrieve{ firstResult in
                sut.retrieve{ secondResult in
                    ...
                    default:
                        XCTFail("Expected to retrieving twice from non emoty cache to deliver save found result, got \(firstResult) and \(secondResult) instead")
                    }
                    exp.fulfill()
                }
            }
    ...
    private func expect(_ sut: CodableFeedStore, toRetrieve expectedReuslt: RetrieveCachedFeedResult, file: StaticString = #file, line: UInt = #line){
        let exp = expectation(description: "Wait for cache retrieval")
